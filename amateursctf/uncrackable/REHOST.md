...
The challenge artifacts for this entry are stored outside the git repo at `../ctf-archive-external/amateursctf/uncrackable/` relative to the `ctf-archive` repo root.
Copy that bundle into the target deployment directory before launching the challenge. A typical pwn.college layout is `/challenge/uncrackable/`, but keep the copied file tree identical to the archived bundle.
