...
target = process('/challenge/babyheap', env={"LD_PRELOAD": "/challenge/libc-2.23.so"}) # The ld_preload is used to switch out the libc version we are using
# gdb.attach(target)
elf = ELF('/challenge/libc-2.23.so')
