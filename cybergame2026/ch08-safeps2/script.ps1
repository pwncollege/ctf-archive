...
$FLAG = [System.IO.File]::ReadAllText("/flag").Trim()

Write-Host "Welcome to SAFEPSv2, hardened environment." -ForegroundColor Cyan
...
function Show-About {
  Write-Host "SAFEPSv2" -ForegroundColor Green
}
...
function Run-JailCommand {
  ...
  foreach ($b in $Bad) {
    ...
    if ($lower -match [regex]::Escape($b)) {
      Write-Host "Nope. $b" -ForegroundColor Red
      return
    }
  }
  ...
  switch ($first) {
    ...
    "echo"  {
      ...
      $sb = [ScriptBlock]::Create("echo $exprTrimmed")
      $result = & $sb
