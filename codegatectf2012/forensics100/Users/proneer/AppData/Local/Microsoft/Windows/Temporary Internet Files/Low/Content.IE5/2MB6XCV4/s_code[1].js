...
var s_account="msie-addons-prod-global";
var locale='';
if ((window.location.host.indexOf('staging.ie8gallery.com') != -1) || (window.location.host.indexOf('localhost') != -1)) {
     s_account='msie-addons-dev';
} else if (document.URL.indexOf('/')!=-1) {
     var tmp=document.URL.substring(document.URL.indexOf(window.location.host));
     locale = tmp.substr(tmp.indexOf('/') + 1, 2);

	 //this switch exists because there was a racent bug fix that changed some of the country codees. 
	 //we need to make sure that the data goes to the correct omniture report suite id, thus the switch
     switch (locale) {
     	case 'us':
     		locale = 'en';
     		break;
...
if (locale) {
     s_account+=",msie-addons-prod-"+locale;
    }
/****WRITING CODE LIKE THIS MAKES MY BRAIN IMPLODE*********/
    if (window.location.href.indexOf('/staples') > -1) {
        s_account = 'msie-staples-prod-en';
    }
    else if (window.location.href.indexOf('cf/partners/bestbuy') > -1 || window.location.href.indexOf('ca/partners/bestbuy') > -1) {
        s_account = 'msie-bestbuy-prod-ca';
    }
    else if (window.location.href.indexOf('geeksquadprecinct') > -1 || window.location.href.indexOf('geeksquad') > -1) {
        s_account = 'msie-bestbuy-prod-en';
    }
    else if (window.location.href.indexOf('/futureshop') > -1) {
        s_account = 'msie-futureshop-prod-ca';
    }
    else if (window.location.href.indexOf('_sony') > -1) {
        s_account = 'msie-sony-prod-eu';
    }
...
s.linkInternalFilters="javascript:,ieaddons.com,ie8gallery.com"
...
function s_doPlugins(s) {
	...
	if (s.pageType=='errorPage') {
		s.pageName='404:'+s.pageName;
	}

	// add-on uploads as engagement
	if (s.events && s.events.indexOf('event4')!=-1) { 
		s.eVar2='resource uploaded';
		s.events=s.apl(s.events,"event2",",",2);
	}

	// internal campaigns / marquees
	if (!s.products) {
		s.eVar1=s.getQueryParam('marquee');
		if (s.eVar1) {
			s.products=";"+s.eVar1;
	...
	// browser version
	s.eVar3=s.getValOnce(detectBrowser(),"s_eVar3",0);

	// OS version
	s.eVar4=s.getValOnce(detectOS(),"s_eVar4",0);

	// default web slice click-throughs
	if (s.getQueryParam('source').toLowerCase()=='webslice') {
	...
	if (s.prop5) {
		...
		var tSearch=s.getValOnce(s.eVar5,"s_eVar5",0);
		if(tSearch){
	...
	if (s.prop11.indexOf("?")!=-1) {
		s.prop11=s.prop11.substring(0, document.URL.indexOf("?"));
	}

	// direct influence pages
	s.eVar9=s.pageName;
	
	var exitTest=s.exitLinkHandler();
	if(exitTest){
		s.linkTrackVars='None';
		s.linkTrackEvents='None';
	}
...
/*
 * Plugin: detectOS (detect user's operating system)
 */
function detectOS() {
	var versions = new Array(
		{ id:'Windows NT 6.0', name:'Windows Vista' },
...
/*
 * Plugin: detectBrowser (detect user's browser version)
 */
function detectBrowser() {
	...
	switch (uaMatch.name) {
		...
		case 'Google Chrome':
			// must check for Google Chrome first since it's ua contains "Safari"
			return uaMatch.name;
		case 'Safari':
			var v='Version/';
