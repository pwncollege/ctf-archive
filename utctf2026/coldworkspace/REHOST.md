...
The challenge artifacts for this entry are stored outside the git repo at `../ctf-archive-external/utctf2026/coldworkspace/` relative to the `ctf-archive` repo root.
Copy that bundle into the target deployment directory before launching the challenge. A typical pwn.college layout is `/challenge/coldworkspace/`, but keep the copied file tree identical to the archived bundle.
